...
public class FileListServiceImpl extends BaseServiceImpl<FileList,Long> implements FileListService {
    ...
    public void create(String path) {
        ...
        FileListPojo fileListPojo = BaiDuUtils.fileList(token, path, 0);
    ...
    private void createByJdbc(FileListPojo.ListDTO fileList,FileList parent) {
        int update = jdbcTemplate.update("insert into filelist(" +
                        ...
                        "orders, category, fileName, fsId, grade, localCTime, localMTime, needUpdate, path, playUrl, serverCTime, serverMTime, treePath, parent_id) value (" +
                        "NOW(),NOW(),0" +
    ...
    private FileList create(FileListPojo.ListDTO fileList) {
        ...
        if(current.isNew()){
            current.setNeedUpdate(fileList.getCategory() == 6);
